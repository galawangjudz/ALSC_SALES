          ...
          <li class="nav-item">
            <div class="btn-group nav-link">
                  <button type="button" class="btn btn-rounded badge badge-light dropdown-toggle dropdown-icon" data-toggle="dropdown">
                    <span><?php 
                    $avatar_path = $_settings->userdata('avatar');
                    $first_dot_position = strpos($avatar_path, '.');
                    if ($first_dot_position !== false) {
                        $avatar_path = substr_replace($avatar_path, '', $first_dot_position, 1);
                    }
                ?>
                    <span><img src="<?php echo validate_image($avatar_path) ?>" class="img-circle elevation-2 user-img" alt="User Image"></span>
